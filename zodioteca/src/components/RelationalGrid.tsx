  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-pink-50 to-purple-50 dark:from-pink-900/20 dark:to-purple-900/20 p-6 rounded-lg">
        <h3 className="text-xl font-semibold mb-2 text-pink-900 dark:text-pink-200 flex items-center gap-2">
        ...
        <p className="text-gray-700 dark:text-gray-300 mb-3">
          ...
          Su objetivo no es "predecir si la relación es buena o mala", sino <strong>entender las dinámicas</strong>, 
          las áreas de atracción, los desafíos y las oportunidades de crecimiento en el vínculo.
        </p>
        <div className="bg-white/50 dark:bg-gray-800/50 p-4 rounded-lg mt-3">
          <h4 className="font-semibold text-gray-800 dark:text-gray-200 mb-2">🌟 Resumen Comparativo</h4>
          <ul className="text-sm space-y-1 text-gray-700 dark:text-gray-300">
            <li><strong>Sinastría:</strong> Cómo se llevan dos cartas entre sí → Dinámica persona a persona</li>
            <li><strong>Compuesta:</strong> Carta simbólica de la relación → Identidad de la pareja</li>
            <li><strong>Davison:</strong> Carta real del vínculo en tiempo/espacio → Propósito y destino compartido</li>
            <li><strong>Astrocartografía Relacional:</strong> Dónde en el mundo la pareja se activa</li>
          </ul>
        </div>
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        >
          Complementarias ({additionalCount})
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredTechniques.map((technique) => (
          <button
            ...
            className={`bg-gradient-to-br ${getTechniqueGradient(technique.name)} text-white p-6 rounded-xl shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300 flex flex-col items-center gap-3 text-left`}
          >
            <span className="text-6xl">{technique.symbol}</span>
            <div className="text-center w-full">
              <h3 className="font-bold text-lg leading-tight">{technique.name}</h3>
              <p className="text-xs opacity-90 mt-2 line-clamp-2">{technique.description}</p>
            </div>
            <span className="text-xs bg-white/20 px-3 py-1 rounded-full mt-auto">
