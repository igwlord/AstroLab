  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 p-6 rounded-lg">
        <h3 className="text-xl font-semibold mb-2 text-indigo-900 dark:text-indigo-200 flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {filteredBodies.map((body) => (
          <button
            ...
            className={`bg-gradient-to-br ${getCategoryGradient(body.category)} text-white p-6 rounded-xl shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300 flex flex-col items-center gap-3`}
          >
            <span className="text-6xl">{body.symbol}</span>
            <div className="text-center">
              <h3 className="font-bold text-sm">{body.name}</h3>
              <p className="text-xs opacity-90 mt-1">{body.chakra}</p>
            </div>
          </button>
        ))}
      </div>
