...
export default function NatalChartForm({ defaultValues, onSubmit, onCancel }: NatalChartFormProps) {
  ...
  const [formData, setFormData] = useState<FormValue>(() => {
    ...
    if (draft) {
      try {
        ...
        logger.debug('📝 Cargando borrador en progreso...');
        return parsed;
      } catch {
    ...
    if (lastSubmitted) {
      try {
        ...
        logger.debug('♻️ Cargando últimos datos enviados...');
        return parsed;
      } catch {
        // Ignorar errores de parseo
      }
  ...
  const selectCity = (city: { name: string; lat: number; lon: number; tzId?: string }) => {
    logger.debug('🏙️ Ciudad seleccionada:', city);
    setFormData(prev => {
      const newLocation = {
        ...prev.location,
        city: city.name,
      ...
      logger.debug('📍 Nuevo location:', newLocation);
      return {
        ...prev,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false);
      logger.debug('🚀 Enviando formulario:', finalData);
      logger.debug('📍 Coordenadas finales:', {
      ...
      logger.debug('💾 Datos guardados para próxima vez');
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-4 sm:space-y-5 md:space-y-6 p-3 sm:p-4 md:p-6">
      {/* Notificación de datos cargados */}
      {showLoadedNotification && (
        <div className="bg-green-50 border border-green-200 rounded-lg sm:rounded-xl p-3 sm:p-4 flex items-center gap-2 sm:gap-3 animate-fade-in">
          <div className="text-xl sm:text-2xl">✅</div>
          <div className="flex-1">
            <p className="text-green-800 font-medium text-sm sm:text-base">Datos cargados</p>
            <p className="text-green-600 text-xs sm:text-sm">Se han cargado los últimos datos guardados</p>
          </div>
          <button
            type="button"
      ...
      {/* Header */}
      <div className="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 text-white shadow-lg">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold mb-1 sm:mb-2">✨ Nueva Carta Natal</h1>
            <p className="text-purple-100 text-xs sm:text-sm md:text-base">Ingresa los datos de nacimiento para calcular tu carta astral</p>
          </div>
          {(localStorage.getItem(STORAGE_KEY) || localStorage.getItem(LAST_SUBMITTED_KEY)) && (
            <button
              type="button"
              onClick={handleClearSavedData}
              className="ml-2 sm:ml-4 px-3 py-1.5 sm:px-4 sm:py-2 bg-purple-700 hover:bg-purple-800 rounded-lg text-xs sm:text-sm font-medium transition-colors flex items-center gap-1 sm:gap-2"
              title="Limpiar datos guardados"
            >
              🗑️ <span className="hidden sm:inline">Limpiar datos</span>
            </button>
          )}
        </div>
      </div>
      ...
      {/* Datos Básicos */}
      <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 shadow-lg border border-purple-100">
        <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-purple-900 mb-3 sm:mb-4">👤 Datos Personales</h2>
        ...
        <div className="grid md:grid-cols-2 gap-3 sm:gap-4">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2 sm:px-4 sm:py-2.5 text-sm sm:text-base border rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent ${
          ...
          {/* Apellido */}
          <div>
            <label htmlFor="surname" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">
              Apellido (opcional)
            </label>
            <input
              id="surname"
              ...
              className="w-full px-3 py-2 sm:px-4 sm:py-2.5 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
        ...
        <div className="mt-3 sm:mt-4">
          <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Género</label>
        ...
        <div className="mt-3 sm:mt-4">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-gray-700 text-sm sm:text-base">Esta carta es para mí</span>
          </label>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 shadow-lg border border-purple-100">
        <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-purple-900 mb-3 sm:mb-4">📅 Fecha y Hora de Nacimiento</h2>
        
        {/* Fecha */}
        <div className="grid grid-cols-3 gap-2 sm:gap-3 md:gap-4 mb-3 sm:mb-4">
          <div>
            <label htmlFor="day" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">
              Día *
            </label>
            <input
              id="day"
              ...
              className={`w-full px-2 py-2 sm:px-3 sm:py-2.5 text-sm sm:text-base border rounded-lg focus:ring-2 focus:ring-purple-500 ${
                errors['birth.day'] ? 'border-red-500' : 'border-gray-300'
              }`}
            />
            {errors['birth.day'] && <p className="text-red-600 text-xs sm:text-sm mt-1">{errors['birth.day']}</p>}
          </div>

          <div>
            <label htmlFor="month" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">
              Mes *
            </label>
            <select
              id="month"
              ...
              className="w-full px-2 py-2 sm:px-3 sm:py-2.5 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
            >
          ...
          <div>
            <label htmlFor="year" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">
              Año *
            </label>
            <input
              id="year"
            ...
            />
            {errors['birth.year'] && <p className="text-red-600 text-xs sm:text-sm mt-1">{errors['birth.year']}</p>}
          </div>
        </div>
        ...
        {formData.birth.timeAccuracy !== 'unknown' && (
          <div className="grid grid-cols-2 gap-2 sm:gap-3 md:gap-4">
            <div>
              <label htmlFor="hour" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">
                Hora
              </label>
              <select
                id="hour"
            ...
            <div>
              <label htmlFor="minute" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">
                Minutos
              </label>
              <select
                id="minute"
      ...
      <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 shadow-lg border border-purple-100">
        <div className="flex justify-between items-center mb-3 sm:mb-4">
          <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-purple-900">📍 Ubicación</h2>
          <button
            type="button"
            onClick={(e) => {
              e.preventDefault();
            ...
            className="text-xs sm:text-sm text-purple-600 hover:text-purple-700 font-medium px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg hover:bg-purple-50 transition-colors"
          >
        ...
        {!useManualCoords ? (
          <div className="space-y-3 sm:space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className={`w-full px-3 py-2 sm:px-4 sm:py-2.5 text-sm sm:text-base border rounded-lg focus:ring-2 focus:ring-purple-500 ${
              ...
              {errors['location.countryCode'] && (
                <p className="text-red-600 text-xs sm:text-sm mt-1">{errors['location.countryCode']}</p>
              )}
            </div>
            ...
            {formData.location.countryCode && (
              <div>
                <label htmlFor="region" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">
                  Provincia / Estado
                </label>
                <select
                  id="region"
            ...
            {formData.location.region && (
              <div>
                <label htmlFor="cityQuery" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">
                  Ciudad
                </label>
                <div className="relative">
                  ...
                  {!formData.location.city && (
                    <p className="text-xs text-gray-500 mt-1">
                      💡 Opcional: Se usarán coordenadas de <strong>{formData.location.region}</strong>
                    </p>
                  )}
                </div>
                  ...
                  <div className="mt-2 border border-gray-200 rounded-lg max-h-40 sm:max-h-48 overflow-y-auto">
                    {cities.map((city, idx) => (
                      <button
                        ...
                        className="w-full text-left px-3 py-2 sm:px-4 text-sm sm:text-base hover:bg-purple-50 border-b border-gray-100 last:border-0 transition-colors"
                      >
            ...
            {formData.location.city && (
              <div>
                <label htmlFor="neighborhood" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">
                  Barrio (opcional)
                </label>
                <input
                  id="neighborhood"
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3 md:gap-4">
              <div>
                <label htmlFor="lat" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">
                  Latitud *
                </label>
                <input
                  id="lat"
                ...
                />
                {errors['location.lat'] && (
                  <p className="text-red-600 text-xs sm:text-sm mt-1">{errors['location.lat']}</p>
                )}
              </div>

              <div>
                <label htmlFor="lon" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">
                  Longitud *
                </label>
                <input
                  id="lon"
                ...
                />
                {errors['location.lon'] && (
                  <p className="text-red-600 text-xs sm:text-sm mt-1">{errors['location.lon']}</p>
                )}
              </div>
            </div>
          </div>
            ...
            {manualTz && (
              <div className="mt-3">
                <select
                  ...
                  className="w-full px-3 py-2 sm:px-4 sm:py-2.5 text-sm sm:text-base border border-blue-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                >
      ...
      <div className="flex gap-2 sm:gap-3 md:gap-4">
        {onCancel && (
          <button
            type="button"
            onClick={onCancel}
            className="flex-1 px-4 py-2.5 sm:px-5 sm:py-3 md:px-6 text-sm sm:text-base border-2 border-gray-300 text-gray-700 rounded-lg sm:rounded-xl font-semibold hover:bg-gray-50 transition-colors"
          >
            Cancelar
          </button>
        )}
        <button
          type="submit"
          disabled={loading}
          className="flex-1 px-4 py-2.5 sm:px-5 sm:py-3 md:px-6 text-sm sm:text-base bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg sm:rounded-xl font-semibold hover:from-purple-700 hover:to-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
        >
