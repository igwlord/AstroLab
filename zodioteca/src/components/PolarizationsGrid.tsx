    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 p-6 rounded-lg">
        ...
        <p className="text-gray-700 dark:text-gray-300 mb-3">
          ...
          demasiada fuerza en la carta natal. Puede ser por acumulación de planetas en un mismo signo/elemento, 
          o porque un planeta recibe múltiples aspectos tensos convirtiéndose en un <strong>"foco" de la dinámica</strong>.
        </p>
        <div className="bg-white/50 dark:bg-gray-800/50 p-4 rounded-lg mt-3">
          <h4 className="font-semibold text-gray-800 dark:text-gray-200 mb-2">🎯 Efectos de la Polarización</h4>
          <p className="text-sm text-gray-700 dark:text-gray-300">
            La polarización genera un <strong>desequilibrio energético</strong>: es el área donde la persona 
            vive más aprendizajes y desafíos, y a la vez donde tiene <strong>más potencial de maestría</strong> 
            si logra integrar conscientemente esa energía.
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        >
          Ejemplos Planetarios ({exampleCount})
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {filteredPolarizations.map((polarization) => (
          <button
            ...
            className={`bg-gradient-to-br ${getPolarizationGradient(polarization.name)} text-white p-6 rounded-xl shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300 flex flex-col items-center gap-3 text-left`}
          >
            <span className="text-6xl">{polarization.symbol}</span>
            <div className="text-center w-full">
              <h3 className="font-bold text-sm leading-tight">{polarization.name}</h3>
              <p className="text-xs opacity-90 mt-2">{polarization.chakra}</p>
            </div>
