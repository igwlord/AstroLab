    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 p-6 rounded-lg">
        ...
        <p className="text-gray-700 dark:text-gray-300">
          ...
          conectados mediante aspectos. Estas figuras crean dinámicas especiales en la carta natal que pueden 
          indicar <strong>talentos extraordinarios, desafíos recurrentes o destinos kármicos</strong>. Son como 
          "firmas energéticas" que marcan la vida de forma profunda.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {CONFIGURATIONS.map((config) => (
          <button
            ...
            className={`bg-gradient-to-br ${getConfigGradient(config.name)} text-white p-6 rounded-xl shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-300 flex flex-col items-center gap-3`}
          >
            <span className="text-6xl">{config.symbol}</span>
            <div className="text-center">
              <h3 className="font-bold text-sm leading-tight">{config.name}</h3>
              <p className="text-xs opacity-90 mt-1">{config.chakra}</p>
            </div>
          </button>
        ))}
      </div>
